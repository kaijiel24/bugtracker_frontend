...
const toggleEditTicket = (event) => {
    displayEditTicket.value.toggle(event)
};
        ...
        <div class="col-12 md:col-12 grid align-items-top">
            <div class="col-12 md:col-8">
                ...
                <div class="text-500 mb-2"> raised by
                    <div class="transition-colors transition-duration-300 hover:text-blue-500 hover:underline cursor-pointer inline"
                        @click="toggle">
                        {{ ticket.submitter }}
                    </div> {{
            ...
            <div class="col-12 md:col-4 flex gap-2 mb-4  align-items-top justify-content-start md:justify-content-end">
                <div>
                    <Button label="Edit Ticket" class="p-button-primary" @click="toggleEditTicket" />
                    <OverlayPanel ref="displayEditTicket" appendTo="body" :showCloseIcon="true">
                        <div class="max-w-min">
                            <TicketForm :data="ticketData" />
                        </div>
                    </OverlayPanel>
                </div>
                <Button label="Close Ticket" class="p-button-danger" @click="openConfirmation" />
                <Dialog header="Confirm Close Ticket" v-model:visible="displayConfirmation" :style="{ width: '350px' }"
                    :modal="true">
                    ...
                    <template #footer>
                        ...
                        <Button label="Yes" icon="pi pi-check" @click="confirmCloseTicket" class="p-button-danger"
                            autofocus />
                    </template>
                </Dialog>
            </div>
